...
class TweetTestCase(TestCase) :
    def setUp(self) :
        ...
        self.user_bus = User.objects.create_user(username="abc2", password="somepassword2")
        Tweet.objects.create(content="my first tweet", user=self.user)
        Tweet.objects.create(content="my first tweet", user=self.user)
        Tweet.objects.create(content="my first tweet", user=self.user_bus)
        self.cuuerntCount = Tweet.objects.all().count()
    ...
    def test_action_unlike(self):
        ...
        self.assertEqual(response.status_code, 200)
        unlike_count = response.json().get('likes')
        self.assertEqual(unlike_count, 0)

    def test_action_retweet(self):
        client = self.get_client()
        cuuernt_count = self.cuuerntCount
        ...
        self.assertEqual(cuuernt_count + 1, new_count)
    ...
    def test_tweet_delete_api_view(self) :
        ...
        response_incrorrect_owner = client.delete("/api/tweets/3/delete/")
        self.assertEqual(response_incrorrect_owner.status_code, 401)